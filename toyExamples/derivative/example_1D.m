...
% f          = @(x)  x.^4 + x.^3 - 7 * x.^2 - x + 6;%(x - 2) .* (x - 1) .* (x + 1) .* (x + 3) ./ 10; df = 0 at x = [-2.2232e+00, 1.6126e+00 -1.3946e-01]
...
sv         = 0.1;
x          = linspace(-2, 2, ntrain);%[rand(1,ntrain)*2 - 1].*5;%linspace(-10, 10, ntrain);%-1:0.1:1;
% x(x>1|x<-1) = [];
% ntrain = size(x, 2);
Sx         = .001*ones(ntrain, 1);

% idx        = randperm(length(x), ntrain+ntest);
% x          = x(idx)';
...
xtrainOrg  = xtrain;
% xtest      = x(idxTest, :);
% xtrain     = [rand(ntrain, 1)*2 - 1]; % Generate covariate between [-1, 1];
xtest      = [0 0 0 0 0]';
ytrainTrue = f(xtrain);
...
% [xopt1]=task.runOptimization_V2(net, xtrain, ytrain, xtest, ytest, 0, 1, x0, Sx0); % function -> Layer-wise opt
% [xopt1]=task.runOptimization_V3(net, xtrain, ytrain, xtest, ytest, 0, 1, x0, Sx0); % function -> direct opt
% [xopt1]=task.runOptimization_V4(net, xtrain, ytrain, xtest, ytest, 0, 1, x0, Sx0); % simultaneously direct opt
if normalizeData
