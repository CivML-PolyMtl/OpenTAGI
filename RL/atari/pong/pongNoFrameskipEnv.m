classdef pongNoFrameskipEnv < rl.env.MATLABEnvironment
    % This code is based on Paulo Carvalho's code (2019)    
    properties
        % Select environment name here 
        open_env = py.gym.make('PongNoFrameskip-v4'); 
    end
    methods              
        function this = pongNoFrameskipEnv()
            % Initialize Observation settings
            ObservationInfo             = rlNumericSpec([4 1]);
            ObservationInfo.Name        = 'pong game';
            ObservationInfo.Description = 'images';         
            ActionInfo                  = rlFiniteSetSpec([0 1 2 3 4 5]); 
            ActionInfo.Name             = 'noop, fire, left, right, left fire, right fire';            
