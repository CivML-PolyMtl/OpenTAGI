...
classdef task
    methods (Static)   
        % Visual field (under development)
        function runImageGenerationVFtrain(net, x, y, trainedModelDir,...
                initModelDir, initEpoch)
            % Initialization          
            cdresults  = net.cd;
            ...
            while run
                ...
                % Training
                net.trainMode = true;
                [theta, normStat] = network.imageGenerationVF(net, theta,...
                    normStat, states, maxIdx, x, y, NiterTrain);
                ...
                disp('Testing... ')
                net.trainMode = false;
                [~, ~, img] = network.imageGenerationVF(net, theta,...
                    normStat, states, maxIdx, x, [], NiterVal); 
                timeTest    = timeTest + toc(tt);
                timeTestRem = timeTest / epoch * (net.maxEpoch - epoch) / 60;
                              
                if net.displayMode == 1
                    formatSpec = 'Time left: %1.0f mins\n';
                    fprintf(formatSpec, timeRem+timeTestRem);
                end
        ...
        function runImageGenerationVFtrain_V2(net, x, y, trainedModelDir,...
                initModelDir, initEpoch)
            % Initialization          
            cdresults  = net.cd;
            modelName  = net.modelName;
            ...
            while run
                ...
                % Training
                net.trainMode = true;
                [theta, normStat] = network.imageGenerationVF_V2(net, theta,...
                    normStat, states, maxIdx, x, y, NiterTrain);
                trainTime = trainTime + toc(ts);
                timeRem   = trainTime / epoch * (net.maxEpoch - epoch) / 60;
                tt = tic;
                ...
                disp('Testing... ')
                net.trainMode = false;
                [~, ~, img] = network.imageGenerationVF_V2(net, theta,...
                    normStat, states, maxIdx, x, [], NiterVal); 
                ...
                imgRef  = y(1 : NiterVal,:)';
                imgPlot = [img, imgRef]';
                pl.vflatenVariables(imgPlot, net.imgSize, net.numlv + 1, NiterVal)
                pause(0.01)
                timeTest    = timeTest + toc(tt);
                timeTestRem = timeTest / epoch * (net.maxEpoch - epoch) / 60;
        ...
        function runVfImageGenerationFullCov(net, x, Sx, y, trainedModelDir,...
                initModelDir, initEpoch)
            % Initialization          
            cdresults  = net.cd;
            modelName  = net.modelName;
            ...
            % Full covariance
            if net.cuda
                Sxlvf = inf * ones(net.nx, net.nx, 'like', x);
                Sxlvf = tril(Sxlvf);
                Sxf = inf * ones(net.nx - net.numlv, net.nx - net.numlv, 'like', x);
                Sxf = tril(Sxf);% + diag(nan(net.nx - net.numlv, 1));
%                 Sxf(isnan(Sxf)) = inf;
                
                Slvf  = inf * ones(net.numlv, net.numlv, 'like', x);
                Slvf  = tril(Slvf) + diag(nan(net.numlv, 1));
                
                Sxlvf(1:net.nx - net.numlv, 1:net.nx - net.numlv) = Sxf;
                Sxlvf(net.nx - net.numlv + 1:end, net.nx - net.numlv + 1 : end) = Slvf;
                
                Sxlvf = Sxlvf(Sxlvf ~= 0); 
                Sxlvf(isinf(Sxlvf)) = 0;                                
                lvIdx = find(isnan(Sxlvf));
            else
                Sxf   = (net.sx.^2) * eye(net.nx-net.numlv, net.nx-net.numlv, 'like', x);
                Slvf  = diag(nan * ones(net.numlv, 1, 'like', x));
                Sxlvf = blkdiag(Sxf, Slvf);
                Sxlvf = Sxlvf(:);
                lvIdx = find(isnan(Sxlvf));
            end
            net.trainMode = true;
            ...
            while run
                ...
                % Training
                net.trainMode = true;
                [theta, normStat] = network.vfImageGenerationFullCov(net,...
                    theta, normStat, states, maxIdx, x, Sx, Sxlvf, lvIdx,...
                    y, NiterTrain);
                trainTime = trainTime + toc(ts);
                timeRem   = trainTime / epoch * (net.maxEpoch - epoch) / 60;
                tt = tic;
                ...
                disp('Testing... ')
                net.trainMode = false;
                [~, ~, img] = network.vfImageGenerationFullCov(net, theta,...
                    normStat, states, maxIdx, x, Sx, Sxlvf, lvIdx, [],...
                    NiterVal); 
        ...
        function runImageGenerationVFtest(net, x, y, trainedModelDir,...
                initModelDir)
            % Initialization          
            NiterVal   = 1;
        ...
        function runVFclassification(net, x, ytrain, ytest, labelTest, ...
            ...
            savedEpoch = net.savedEpoch;
            maxEpoch   = net.maxEpoch;
            NiterTrain = size(ytrain, 1);
            NiterTest  = size(ytest, 1);
            erTest     = zeros(NiterTest, net.maxEpoch, net.dtype);
            PrTest     = zeros(NiterTest, net.numClasses, net.maxEpoch,...
                net.dtype);
            ...
            while run
                ...
                net.trainMode = true;
                [thetaClass,...
                    normStatClass] = network.vfClassificationTrain(net,...
                    theta, normStat, states, maxIdx, x, ytrain, NiterTrain);
                trainTime = trainTime + toc(ts);
                timeRem   = trainTime / epoch * (net.maxEpoch - epoch) / 60;
                tt = tic;
                ...
                net.trainMode = false;
                [PrTest(:,:,epoch),...
                    erTest(:, epoch)] = network.vfClassificationTest(net,...
                    thetaClass, normStatClass, states, maxIdx, x, ytest, ...
                    classObs, classIdx, labelTest, NiterTest);               
        ...
        function DQN1net(netP, env, trainedModelDir, startEpisode)
            % Initialize nets           
            netP1              = netP;
            netP1.batchSize    = 1;
            ...
            cdresults     = netP.cd;
            savedEpisode  = netP.savedEpisode;
            numEpisodes   = netP.rl.numEpisodes;  
            initialMemory = netP.rl.initialMemory;   
            dtype         = netP.dtype;
            B             = netP.batchSize;
            rB            = netP.repBatchSize;
            maxMemory     = netP.rl.maxMemory;
            gpu           = netP.gpu;
            numEp4avg    = 100;
            stat       = zeros(numEpisodes, 6, netP.dtype);
            memory     = replayMemory.init(maxMemory, dtype); 
            ...
            if isempty(trainedModelDir)                  
                ...
                stat      = zeros(numEpisodes+startEpisode, 1, netP.dtype);
                stat(1:startEpisode, :) = l.stat(1:startEpisode, :);
                clear l
            end
            thetaT = thetaP;
            netP.trainMode  = true; 
            netP1.trainMode = false;
            
            % POMDP
            if netP.fullcov
                Ssd = (netP.sx.^2) * ones(netP.nx, 1, netP.dtype);
                Ssf = ones(netP.nx, netP.nx, netP.dtype);
                Ssf = tril(Ssf) + diag(nan(netP.nx, 1));
                Ssf = Ssf(Ssf~=0);
                Ssf(Ssf==1) = 0;
                Ssf(isnan(Ssf)) = netP.sx.^2;
                SsfBatch = repmat(Ssf, [netP.batchSize, 1]);
                SsdBatch = repmat(Ssd, [netP.batchSize, 1]);
            else
                Ssd = nan;
                Ssf = nan;
                SsfBatch = nan;
            ...
            disp('Training...')
            disp(' ')
            for ep = startEpisode + 1 : startEpisode + numEpisodes      
                % Initialize episodes
                obs    = env.reset(); 
                state  = obs(:); 
                if netP.PO
                    state = state + normrnd(zeros(size(state, 1), 1),...
                        netP.sx * ones(size(state, 1), 1));
                end
                state  = cast(state, dtype);
                run    = 1;
                ts     = tic;
                loop   = 0;
                loopUd = 0;               
                ...
                while run     
                    ...
                    loopUd   = loopUd + 1;
                    numSteps = numSteps + 1;   
                    % Decay observation nosie sv
                    if mod(numSteps, 128) == 0
                    ...
                    % Select action
                    if netP.fullcov
                        action = rltagi.getAction1netFullCov(netP1, thetaP,...
                            statesP1, normStatP1, maxIdxP1, state, Ssd, Ssf);
                    else
                        action = rltagi.getAction1net(netP1, thetaP,...
                            statesP1, normStatP1, maxIdxP1, state);
                    end
                    ...
                    if ~done
                        ...
                        if netP.PO
                            nextState = nextState +...
                                normrnd(zeros(size(nextState, 1), 1),...
                                netP.sx * ones(size(nextState, 1), 1));
                        end
                    else
                    ...
                    % Save states
                    action    = cast(action, dtype);
                    nextState = cast(nextState, dtype);
                    state     = cast(state, dtype);
                    reward    = cast(reward, dtype);
                    [memory, count, filledIdx,...
                        fullMem] = replayMemory.push(memory, state, action,...
                        nextState, reward, finalState, count, maxMemory,...
                        fullMem, filledIdx);
                    state = nextState;
                    
                    % Train nets
                    if filledIdx>=initialMemory&&loopUd>=rB*B
                        [stateBatch, actionBatch, nextStateBatch,...
                         rewardBatch, finalStateBatch] = replayMemory.sample(memory,...
                         rB * B, filledIdx);
                        if gpu
                            stateBatch     = gpuArray(stateBatch);
                            actionBatch    = gpuArray(actionBatch);
                            nextStateBatch = gpuArray(nextStateBatch);
                            rewardBatch    = gpuArray(rewardBatch);
                            SsfBatch       = gpuArray(SsfBatch);
                            SsdBatch       = gpuArray(SsdBatch);
                        end
                        if netP.fullcov && netP.cuda && netP.gpu
                            [thetaP, ...
                             normStatP] = network.DQN1netFullCovCUDA(netP,...
                             thetaP, thetaT, statesP, normStatP, maxIdxP,...
                             stateBatch, SsdBatch, SsfBatch, actionBatch,...
                             nextStateBatch, SsdBatch, SsfBatch, rewardBatch,...
                             finalStateBatch);
                        else
                            [thetaP, normStatP] = network.DQN1net(netP,...
                             thetaP, thetaT, statesP, normStatP, maxIdxP,...
                             stateBatch, actionBatch, nextStateBatch,...
                             rewardBatch, finalStateBatch);
                        end
                        thetaT = thetaP;
                    end
                    if done; break; end
                ...
                teEp = toc(ts);
                te = te + teEp;
                tr = te / numSteps * (netP.rl.maxSteps - numSteps ) / 60;
                % Save model
                stat(ep, 1) = episodeReward;
                stat(ep, 2) = loop;
                stat(ep, 3) = teEp/60;
                stat(ep, 4) = mean(stat(max(1, ep - numEp4avg) : ep, 1));
                stat(ep, 5) = mean(stat(max(1, ep - numEp4avg) : ep, 2));
                stat(ep, 6) = episodeRewardClip;            
                if mod(ep, savedEpisode) == 0 ...
                        || ep == (numEpisodes + startEpisode)
                    task.saveRLDQN1net(cdresults, modelName, dataName,...
                        thetaP, normStatP, stat, netPinfo, ep);
                end
                if numSteps>=netP.rl.maxSteps 
                    stat = stat(1 : ep, :);
                    task.saveRLDQN1net(cdresults, modelName, dataName,...
                        thetaP, normStatP, stat, netPinfo, ep);
                    break
                ...
                if mod(ep, 20) == 0
                    ...
                    fprintf(formatSpec, numSteps, netP.rl.maxSteps,...
                        stat(ep, 1), stat(ep, 4), stat(ep, 5), tr);
                end
            end
            disp('Done.')
        end
        function runDQNCARL(netQ, netA, env, trainedModelDir, startEpisode)
            % Hyperparameters
            ...
            if isempty(trainedModelDir) 
                ...
                stat      = zeros(numEpisodes, 5, dtype);
                statS     = zeros(floor(maxNumSteps / netQ.rl.stepUpdate),...
                    4, dtype);
                self.mrew = 0;
                self.Srew = 1;
                self.countRew   = 1E-4;
            ...
            for ep = startEpisode+1:startEpisode+numEpisodes      
                ...
                % Select action 
                if gpu; stateA = gpuArray(state); end  
                [action] = rltagi.getContAction1net(netA1, stateA);
                while run     
                    loop       = loop + 1;
                    loopUd     = loopUd + 1;
                    ...
                    % Decay observation nosie sv
                    if mod(totalSteps, netQ.rl.stepUpdate)==0
                        loopStep = loopStep + 1;
                        ...
                        netQ1.sv = netQ.sv;
                        netA1.sv = netA.sv;
                        % Save data w.r.t time steps
                        statS(loopStep, 1) = totalSteps;
                        statS(loopStep, 2) = tt / 60; %[mins]
                        statS(loopStep, 3) = mean(stat(max(1, ep - numEp4avg) : ep, 1));
                        statS(loopStep, 4) = mean(stat(max(1, ep - numEp4avg) : ep, 2));
                    end
...
%                     
                    if ~done
                        ...
                        nextState    = (nextState - self.ms) ...
                            ./ (sqrt(self.Ss) + 1E-8);
                        nextState    = rltagi.clamp(nextState, ...
                            netQ.rl.obsLow, netQ.rl.obsHigh);
                        if gpu; nextStateA = gpuArray(nextState); end  
                        [nextAction] = rltagi.getContAction1net(netA1, ...
                            nextStateA);
                        nextAction   = gather(nextAction);
                        finalState   = false;
                    else
                        nextState  = zeros(size(state), dtype);
                        nextAction = zeros(size(action), dtype);
                        finalState = true;
                    end              
                    
                    % Save states
                    action     = cast(action, dtype);
                    nextAction = cast(nextAction, dtype);
                    state      = cast(gather(state), dtype);
                    nextState  = cast(nextState, dtype);
                    reward     = cast(reward, dtype);
                    [memory, count, fullMem,...
                        filledIdx] = replayMemory.push_V2(memory, state, ...
                        action, nextState, nextAction, reward, finalState, ...
                        count, maxMemory, fullMem, filledIdx);
                    state  = nextState;
                    action = nextAction;
                    
                    % Train nets
                    if filledIdx >= B * rB%netQ.rl.initialMemory
                        update = true;
                        loopEpoch = 0;
                        while update
                            loopEpoch  = loopEpoch +1;
                            [stateBatch, actionBatch, nextStateBatch,...
                             nextActionBatch, rewardBatch,...
                             finalStateBatch] = replayMemory.sample_V2(memory,...
                             rB * B, filledIdx);
                            if gpu
                                stateBatch      = gpuArray(stateBatch);
                                actionBatch     = gpuArray(actionBatch);
                                nextStateBatch  = gpuArray(nextStateBatch);
                                nextActionBatch = gpuArray(nextActionBatch);
                                rewardBatch     = gpuArray(rewardBatch);
                            end
                            [thetaQ, thetaA] = network.DQNCARL(netQ, netA,...
                             stateBatch, actionBatch, nextStateBatch,...
                             nextActionBatch, rewardBatch, finalStateBatch);
                            netQ.theta  = thetaQ;
                            netA.theta  = thetaA;
                            if loopEpoch >= netQ.rl.noptepochs
                                break;
                            end
                        end
                    end
                ...
                stat(ep, 4) = mean(stat(max(1,ep-numEp4avg):ep, 1));
                stat(ep, 5) = mean(stat(max(1, ep-numEp4avg):ep, 2));              
%                 if mod(ep, savedEpisode)==0||ep==(numEpisodes+startEpisode)
%                     task.saveRLDQN1net(cdresults, modelName, dataName, thetaP, normStatP, stat, netPinfo, ep);
%                 end
                if totalSteps >= maxNumSteps
                    ...
                    task.saveRLDQN2net(cdresults, modelName, dataName, ...
                        ...
                        netA.normStat, netAinfo, statS, ep+startEpisode)
                    break
                end
                
                % Display results
                if mod(ep, logInterval) == 0
                    formatSpec = 'Step# %4.2e/%0.0e, Reward: %7.1f, Avg. reward: %7.0f, steps: %4.0f, rew/step: %3.1f, Time left: %1.0f mins\n';
                    fprintf(formatSpec, totalSteps, maxNumSteps, stat(ep, 1),...
                        stat(ep, 4), stat(ep, 5), stat(ep, 4)/stat(ep, 5), tr);
                end
            end
            disp('Done.')
...
%                 % Collect data
        ...
        function DQN1netWithImagesCUDA(netP, env, trainedModelDir, startEpisode)
            % Initialize nets
            modelName     = netP.modelName;
            dataName      = netP.dataName;
            cdresults     = netP.cd;
            ...
            if isempty(trainedModelDir)                  
                thetaP    = tagi.initializeWeightBias(netP); 
                normStatP = tagi.createInitNormStat(netP);              
                memory    = replayMemory.init(netP.rl.maxMemory, dtype); 
            else
                l         = load(trainedModelDir);
                thetaP    = l.theta;
                normStatP = l.normStat;
            ...
            filledIdx = 0;
            count     = 0;
            stateS0     = zeros(prod(netP.imgSize(1:2)), 1, 'single');
            stateM0     = zeros(netP.imgSize, 'single', 'gpuArray');
            action0     = cast(1, dtype);
            rewardclip0 = cast(0, dtype);
            finalState0 = cast(0, dtype);
            ...
            isfire      = isfire && length(meanings) > 3;
            wasRealDone = true;
            disp('Training...')
            disp(' ')
            for ep = startEpisode + 1 : startEpisode + numEpisodes      
                % Initialize episodes                             
                if isfire && wasRealDone
                    [obs, action0] = wrappers.noopFireResetEnv(env);
                    stateM = stateM0;
                    
                    % Store first 3 frames
                    for nf = 1 : numFrames - 1
                        [memory, top, bottom,...
                            currSize] = replayMemory.pushAtari(memory,...
                            stateS0, action0, rewardclip0, finalState0,...
                            maxMemory, top, bottom, currSize);
                    end
                    episodeReward     = 0;
                    episodeRewardClip = 0;
                    loop              = 0;
                elseif ~isfire && wasRealDone
                    obs     = wrappers.noopResetEnv(env);
                    stateM  = stateM0;
                    action0 = 1;
                    ...
                    stateM(:, :, 1 : numFrames - 1) = stateM(:, :, 2 : numFrames);
                    env.step(0);
                    [~, ~, done] = env.step(1);
                    action0 = 2;
                    if ~done
                ...
                img     = cast(img, dtype) / 255;    
                stateS  = reshape(img, [numel(img), 1]);
                [memory, top, bottom,...
                    currSize] = replayMemory.pushAtari(memory, stateS, ...
                    action0, rewardclip0, finalState0, maxMemory, top, ...
                    bottom, currSize);
                lives   = double(env.episodicLife());               
                stateM(:, :, end)  = gpuArray(img);
                stateA  = stateM(:);
                run     = 1;
                ts      = tic;                        
                savedFrame = cell(N, 1);             
                ...
                while run     
                    ...
                    % Select action
                    action = rltagi.getAction1netCUDA(netP1, thetaP,...
                        statesP1, normStatP1, maxIdxP1, stateA);
                    action = gather(action);
                    
                    % Collect observation
                    action2gym = action - 1 ;
                    [stateS, rewardclip, reward, obs,...
                        done] = wrappers.getFrames(env, action2gym, lives,...
                        numFrameskip, resizeMethod);
                    stateM(:, :, 1 : numFrames - 1) = stateM(:, :, 2 : numFrames);
                    stateM(:, :, end) = gpuArray(stateS);
                    stateS = stateS(:);
                    
                    % Episodic life
                    newlives    = double(env.episodicLife());
                    wasRealDone = done;
                    ...
                    % Save frame
                    savedFrame{loop, 1} = obs;                   
                    
                    % Reward
                    episodeReward     = episodeReward + reward;
                    episodeRewardClip = episodeRewardClip + rewardclip;
                    if ~done
                        stateA = stateM(:);
                        finalState = 0;
                    else
                    ...
                    [memory, top, bottom, ...
                        currSize] = replayMemory.pushAtari(memory, stateS,...
                        action, rewardclip, finalState, maxMemory, top, ...
                        bottom, currSize);
                    
                    % Train nets
                    if currSize > B * rB + numFrames
                        [state, action, nextState, reward, finalState,...
                            netUpdate] = replayMemory.sampleAtari(memory,...
                            B * rB, numFrames, bottom, currSize);
                        if netUpdate
                            state     = gpuArray(state);
                            action    = gpuArray(action);
                            nextState = gpuArray(nextState);
                            reward    = gpuArray(reward);
                            [thetaP, normStatP] = network.DQN1netCUDA(netP,...
                                thetaP, thetaT, statesP, normStatP, maxIdxP,...
                                state, action, nextState, reward, finalState);
                            thetaT = thetaP;
                        end
                    ...
                    if numSteps >= netP.rl.maxNumSteps
                        stat(ep, 1) = episodeReward;
                        stat(ep, 2) = loop;
                        stat(ep, 3) = teEp / 60;
                        stat(ep, 4) = mean(stat(max(1, ep - numEp4avg) : ep, 1));
                ...
                teEp = toc(ts);
                te   = te + teEp;
                tr   = te / numSteps * (netP.rl.maxNumSteps - numSteps) / 60;
                ...
                % Display results
                if mod(ep, netP.logInterval) == 0
                    formatSpec = 'Step# %4.2e/%0.0e, Reward: %7.1f, Avg. reward: %7.1f, steps: %7.0f, Time left: %1.0f mins\n';
                    fprintf(formatSpec, numSteps, netP.rl.maxNumSteps,...
                        stat(ep, 1), stat(ep, 4), stat(ep, 5), tr);
                end
            end
            disp('Done.')
        end 
        function nstepQwithImagesCUDA(netP, env, trainedModelDir, startEpisode)
            % Hyperparameters
            modelName    = netP.modelName;
            dataName     = netP.dataName;
            cdresults    = netP.cd;
