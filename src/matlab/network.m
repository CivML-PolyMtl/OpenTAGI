...
classdef network
    methods (Static)
        % Visual field
        function [theta, normStat, img] = imageGenerationVF(net, theta,...
                normStat, states, maxIdx, x, y, Niter)
            % Initialization           
            numDataPerBatch = net.batchSize*net.repBatchSize;
            numCvr = prod(net.imgSize);
            ...
            if net.trainMode
                ...
                for i = 1:Niter
                    ...
                    loop     = loop + 1;
                    yloop    = y(i, :)';
                    refCoord = randperm(numCvr, numCvr);
                    for k = 1:numDataPerBatch:numCvr
                        idxBatch = k : k + numDataPerBatch - 1;
                        idxloop  = refCoord(idxBatch);
                        xloop    = reshape(x(idxloop, :)',...
                            [net.nx * net.batchSize, net.repBatchSize]);
                        ...
                        [deltaM, deltaS, deltaMx,...
                            deltaSx] = tagi.hiddenStateBackwardPass(net,...
                            theta, normStat, states, yloop(idxloop), [], [],...
                            maxIdx);
                    ...
                    if mod(i, net.obsShow) == 0 && i > 1 ...
                            && net.trainMode == 1 && net.displayMode == 1
                ...
                img = zeros(prod(net.imgSize), 1);
                for i = 1:Niter
                    loop     = loop + 1;
                    refCoord = 1:numCvr;
                    for k = 1 : numDataPerBatch : numCvr
                        idxBatch = k : k + numDataPerBatch - 1;
                        idxCoord = refCoord(idxBatch);
                        xloop    = reshape(x(idxCoord, :)',...
                            [net.nx * net.batchSize, net.repBatchSize]);
                        ...
                        img(idxBatch, :) = reshape(gather(ma{end}),...
                            [net.ny * numDataPerBatch, 1]);
                    end                    
                end
            end
        end
        function [theta, normStat, img] = imageGenerationVF_V2(net, theta,...
                normStat, states, maxIdx, x, y, Niter)
            % Initialization           
            numDataPerBatch = net.batchSize * net.repBatchSize;
            numCvr = prod(net.imgSize);
            ...
            if net.trainMode
                ...
                for i = 1 : Niter
                    ...
                    yloop    = y(i, :)';
                    refCoord = randperm(numCvr, numCvr);
                    mlv      = 2 * rand(1, net.numlv, 'like', yloop) - 1;
                    Slv      = net.slv.^2 * ones(size(mlv), 'like', yloop);
                    for k = 1 : numDataPerBatch : numCvr
                        idxBatch = k : k + numDataPerBatch - 1;
                        idxloop  = refCoord(idxBatch);
                        xloop    = reshape(x(idxloop, :)',...
                            [net.numCoord * net.batchSize, net.repBatchSize]);
                        mlvloop  = reshape(repmat(mlv, [numDataPerBatch, 1])',...
                            [net.batchSize * net.numlv, net.repBatchSize]);
                        Slvloop  = reshape(repmat(Slv, [numDataPerBatch, 1])',...
                            [net.batchSize * net.numlv, net.repBatchSize]);
                        mz0loop  = tagi.attachMeanVar(xloop, mlvloop,...
                            net.numCoord, net.numlv, net.batchSize, net.repBatchSize);
                        Sz0loop  = tagi.attachMeanVar(zeros(size(xloop)),...
                            Slvloop, net.numCoord, net.numlv, net.batchSize,...
                            net.repBatchSize);
                        
                        % Get batch of data
                        states = tagi.initializeInputs(states, mz0loop,...
                            Sz0loop, [], [], [], [], [], [], [], net.xsc);
                        ...
                        [deltaM, deltaS,deltaMx, deltaSx,...
                            ...
                            theta, normStat, states, yloop(idxloop), [], [], maxIdx);
                        [~, deltaMzlv] = tagi.detachMeanVar(deltaMz0, net.numCoord,...
                            net.numlv, net.batchSize, net.repBatchSize);
                        [~, deltaSzlv] = tagi.detachMeanVar(deltaSz0, net.numCoord,...
                            net.numlv, net.batchSize, net.repBatchSize);
                        deltaMzlv = reshape(deltaMzlv, [net.numlv, numDataPerBatch]);
                        deltaSzlv = reshape(deltaSzlv, [net.numlv, numDataPerBatch]);
                        
                        mlv = mlv + sum(deltaMzlv, 2)';
                        Slv = Slv + sum(deltaSzlv, 2)';
                    ...
                    time_tot  = time_tot + time_loop;
                    time_rem  = time_tot / i * (Niter-i) / 60;
                    if mod(i, net.obsShow)==0&&i>1&&net.trainMode==1&&net.displayMode==1
                        disp(['     Time left : ' sprintf('%0.2f',time_rem) ' mins'])
                    end                   
                ...
                img  = zeros(prod(net.imgSize), Niter * net.numlv);
                mlvN = zeros(Niter, net.numlv * net.numlv, 'like', x);
                SlvN = zeros(size(mlvN), 'like', x);
                idxN = colon(1, net.numlv) + colon(0, net.numlv, net.numlv * net.numlv - net.numlv);                              
                mlvN(:, idxN) = repmat(linspace(-2, 2, Niter)', [1, net.numlv]);
                SlvN(:, idxN) = repmat(net.slv.^2, [Niter, net.numlv]);
                mlvN = reshape(mlvN, [net.numlv * Niter, net.numlv]);
                SlvN = 0 * reshape(SlvN, [net.numlv * Niter, net.numlv]);
                for i = 1 : Niter * net.numlv
                    loop     = loop + 1;
                    refCoord = 1:numCvr;
                    mlv      = mlvN(i, :);
                    Slv      = SlvN(i, :);
                    for k = 1:numDataPerBatch:numCvr
                        ...
                        Slvloop  = reshape(repmat(Slv, [numDataPerBatch, 1])',...
                            [net.batchSize * net.numlv, net.repBatchSize]);
                        mz0loop  = tagi.attachMeanVar(xloop, mlvloop, net.numCoord,...
                            net.numlv, net.batchSize, net.repBatchSize);
                        Sz0loop  = tagi.attachMeanVar(zeros(size(xloop)),...
                            Slvloop, net.numCoord, net.numlv, net.batchSize,...
                            net.repBatchSize);
                        ...
                        [states, normStat, maxIdx] = tagi.feedForwardPass(net,...
                            theta, normStat, states, maxIdx);
                        [~, ~, ma, Sa] = tagi.extractStates(states);
                        img(idxBatch, i) = reshape(gather(ma{end}),...
                            [net.ny * numDataPerBatch, 1]);
                    end                    
                end                              
            end
        end
        function [theta, normStat, img] = vfImageGenerationFullCov(net, theta,...
                normStat, states, maxIdx, x, Sx, Sxlvf, lvIdx, y, Niter)
            % Initialization           
            numDataPerBatch = net.batchSize * net.repBatchSize;
            numCvr = prod(net.imgSize);
            ...
            if net.trainMode
                ...
                for i = 1 : Niter
                    ...
                    for k = 1 : numDataPerBatch : numCvr
                        ...
                        xloop    = reshape(x(idxloop, :)', ...
                            [net.numCoord * net.batchSize, net.repBatchSize]);
                        Sxloop   = reshape(Sx(idxloop, :)', ...
                            [net.numCoord * net.batchSize, net.repBatchSize]);
                        Sxlvf(lvIdx) = Slv;
                        Sxlvfloop = repmat(Sxlvf, [net.batchSize, 1]);
                        mlvloop   = reshape(repmat(mlv, [numDataPerBatch, 1])',...
                            [net.batchSize * net.numlv, net.repBatchSize]);
                        Slvloop   = reshape(repmat(Slv, [numDataPerBatch, 1])',...
                            [net.batchSize * net.numlv, net.repBatchSize]);
                        mz0loop   = tagi.attachMeanVar(xloop, mlvloop,...
                            net.numCoord, net.numlv, net.batchSize, net.repBatchSize);
                        Sz0loop   = tagi.attachMeanVar(Sxloop, Slvloop,...
                            net.numCoord, net.numlv, net.batchSize, net.repBatchSize);
                        ...
                        if net.cuda
                            [states, normStat,...
                             maxIdx] = tagi.feedForwardPassFullCovCUDA(net,...
                             theta, normStat, states, maxIdx, Sxlvfloop);
                        else
                            [states, normStat,...
                             maxIdx] = tagi.feedForwardPassFullCov(net, ...
                             theta, normStat, states, maxIdx, Sxlvfloop);
                        end
                        % Update parameters
                        [deltaM, deltaS,deltaMx, deltaSx,...
                         deltaMz0, deltaSz0] = tagi.hiddenStateBackwardPass(net,...
                         theta, normStat, states, yloop(idxloop), [], [], maxIdx);
                        [~, deltaMzlv] = tagi.detachMeanVar(deltaMz0, ...
                            net.numCoord, net.numlv, net.batchSize, net.repBatchSize);
                        [~, deltaSzlv] = tagi.detachMeanVar(deltaSz0, ...
                            net.numCoord, net.numlv, net.batchSize, net.repBatchSize);
                        deltaMzlv = reshape(deltaMzlv, [net.numlv, numDataPerBatch]);
                        deltaSzlv = reshape(deltaSzlv, [net.numlv, numDataPerBatch]);
                ...
                for i = 1 : Niter * net.numlv
                    ...
                    for k = 1 : numDataPerBatch : numCvr
                        ...
                        Slvloop   = reshape(repmat(Slv, [numDataPerBatch, 1])',...
                            [net.batchSize * net.numlv, net.repBatchSize]);
                        mz0loop   = tagi.attachMeanVar(xloop, mlvloop,...
                            net.numCoord, net.numlv, net.batchSize,...
                            net.repBatchSize);
                        Sz0loop   = tagi.attachMeanVar(Sxloop, Slvloop,...
                            net.numCoord, net.numlv, net.batchSize,...
                            net.repBatchSize);
        ...
        function [thetaClass, normStatClass] = vfClassificationTrain(net,...
                theta, normStat, states, maxIdx, x, y, Niter)
            % Initialization           
            numDataPerBatch = net.batchSize * net.repBatchSize;
            numCvr          = prod(net.imgSize);
            thetaClass      = cell(net.numClasses, 1);
            normStatClass   = cell(net.numClasses, 1);
            ...
            % Training
            for c = 1:net.numClasses
                thetaloop    = theta;
                normStatloop = normStat;
                for i = 1 : Niter
                    timeval  = tic;
                    loop     = loop + 1;
                    ...
                    mlv = 2*rand(1, net.numlv, 'like', yloop) - 1;
                    Slv = 0.1 * ones(size(mlv), 'like', yloop);
                    
                    % Scan images using coordinates
                    for k = 1:numDataPerBatch:numCvr
                        idxBatch = k : k + numDataPerBatch - 1;
                        idxloop  = refCoord(idxBatch);
                        ...
                        % Forward pass
                        [states, normStat, maxIdx] = tagi.feedForwardPass(net,...
                            thetaloop, normStatloop, states, maxIdx);
                        
                        % Update hidden states
                        [deltaM, deltaS, deltaMx, deltaSx,...
                         deltaMz0, deltaSz0] = tagi.hiddenStateBackwardPass(net, ...
                         thetaloop, normStatloop, states, yloop(idxloop),...
                         [], [], maxIdx);
                        ...
                        [~, deltaSzlv] = tagi.detachMeanVar(deltaSz0, ...
                            net.numCoord, net.numlv, net.batchSize, ...
                            net.repBatchSize);
                        deltaMzlv = reshape(deltaMzlv, ...
                            [net.numlv, numDataPerBatch]);
                        deltaSzlv = reshape(deltaSzlv, ...
                            [net.numlv, numDataPerBatch]);
                        ...
                        % Update parameters
                        dtheta = tagi.parameterBackwardPass(net, thetaloop,...
                            normStatloop, states, deltaM, deltaS, deltaMx, deltaSx);
                        thetaloop = tagi.globalParameterUpdate(thetaloop,...
                            dtheta, net.gpu);
                    end
                    
                    time_loop = toc(timeval);
                    time_tot  = time_tot + time_loop;
                    time_rem  = time_tot / i * (Niter - i) * (net.numClass - c) / 60;
                    if mod(i, net.obsShow) == 0 && i > 1 && net.trainMode && net.displayMode
                        disp(['     Time left : ' sprintf('%0.2f',time_rem) ' mins'])
                    end
                ...
                % Store results for each network
                thetaClass{c}    = thetaloop;
                normStatClass{c} = normStatloop;
            end
        end
        function [prob, er] = vfClassificationTest(net, thetaClass,...
                normStatClass, states, maxIdx, x, y, classObs, classIdx,...
                labels, Niter)
            numDataPerBatch = net.batchSize*net.repBatchSize;
            numCvr          = prod(net.imgSize);           
            stateClass      = cell(net.numClasses, 1);
            prob = zeros(Niter, net.numClasses, net.dtype);
            er   = zeros(Niter, 1, net.dtype);
            for i = 1 : Niter
                % Data and coordinates
                yloop    = y(i, :)';
                refCoord = randperm(numCvr, numCvr);
                
                % Initialize latent variables [-1, 1]
                mlv      = 2*rand(1, net.numlv, 'like', yloop) - 1;
                Slv      = 0.1 * ones(size(mlv), 'like', yloop);
                mlv      = repmat(mlv, [net.numClasses, 1]);
                Slv      = repmat(Slv, [net.numClasses, 1]);
                
                % Initialize gate's parameters
                [mwg, Swg] = tagi.initializeGateParameter(net.levelNodes,...
                    net.dtype);
                ...
                for k = 1:numDataPerBatch:numCvr   
                    ...
                    % Predict pixel 
                    [malClass, SalClass, ...
                        stateClass] = network.pixelPrediction(net, thetaClass,...
                        normStatClass, stateClass, states, maxIdx, xloop,...
                        mlv, Slv);
                    
                    % Update gate's parameters
                    [deltaMzl, deltaSzl,...
                     mwg, Swg] = network.hierarchicalBinary(malClass,...
                     SalClass, mwg, Swg, levelNodes, yloop(idxloop), B,...
                     net.gpu);
                    
                    % Update latent variables
                    [mlv, Slv] = network.latentVariableInference(net,...
                        thetaClass, normStatClass, stateClass, maxIdx, mlv,...
                        Slv, deltaMzl, deltaSzl);                                        
                end
                
                % Compute probability for each class
                mhs        = flipup(mat2cell(mwg));
                Shs        = flipup(mat2cell(Swg));
                classProb  = tagi.obs2class(mhs, Shs, classObs, classIdx);
                classProb  = gather(classProb);
                
                % Store results
                prob(i, :) = classProb;
                er(i, :)   = mt.errorRate(labels', classProb');
            end
        end
        function [deltaMz, deltaSz, mwg, Swg] = hierarchicalBinary(malClass,...
                SalClass, mwg, Swg, levelNodes, y, B, gpu)
            % Forward pass
            [mz, Sz, prob] = tagi.vfFeedForwardPass(malClass, SalClass, mwg,...
                Swg, levelNodes, B);
            
            % Update last layer
            [deltaMzl, deltaSzl] = tagi.fowardHiddenStateUpdate(mypred,...
                Sypred + Sr + netQ.sv.^2, CzQy, y, gpu);
            
            % Update hidden states
            [deltaMz, deltaSz, deltaM,...
             deltaS] = tagi.vfHiddenStateBackwardPassMain(Sz, prob, deltaMzl,...
             deltaSzl, B);
            
            % Update gate parameters 
            [deltaMwg, deltaSwg] = tagi.vfParameterBackwardPassMain(mz, mwg,...
                Swg, deltaM, deltaS, nodes);
            [mwg, Swg] = tagi.globalGateParameterUpdate(mwg, Swg, deltaMwg,...
                deltaSwg);                        
        end
        function [malClass, SalClass, stateClass] = pixelPrediction(net,...
                thetaClass, normStatClass, stateClass, states, maxIdx, xloop, mlv, Slv)
            malClass = zeros(net.batchSize * net.repBatchSize, ...
                net.numClasses, 'like', xloop);
            SalClass = malClass;
            for c = 1:net.numClasses  
                % Get batch of data
                mlvloop  = reshape(repmat(mlv(c, :), [numDataPerBatch, 1])',...
                    [net.batchSize * net.numlv, net.repBatchSize]);
                Slvloop  = reshape(repmat(Slv(c, :), [numDataPerBatch, 1])',...
                    [net.batchSize * net.numlv, net.repBatchSize]);
                mz0loop  = tagi.attachMeanVar(xloop, mlvloop, net.numCoord,...
                    net.numlv, net.batchSize, net.repBatchSize);
                Sz0loop  = tagi.attachMeanVar(zeros(size(xloop)), Slvloop,...
                    net.numCoord, net.numlv, net.batchSize, net.repBatchSize);
                
                % input state
                stateClass{c} = tagi.initializeInputs(states, mz0loop, ...
                    Sz0loop, [], [], [], [], [], [], [], net.xsc);
                
                % Feed to network
                [stateClass{c}] = tagi.feedForwardPass(net, thetaClass{c}, ...
                    normStatClass{c}, stateClass{c}, maxIdx);
                [~, ~, ma, Sa] = tagi.extractStates(stateClass{c});
                malClass(:, c) = ma{end};
                SalClass(:, c) = Sa{end};
            end
        end
        function [mlv, Slv] = latentVariableInference(net, thetaClass,...
                normStatClass, stateClass, maxIdx, mlv, Slv, deltaMzl, deltaSzl)
            for c = 1:net.numClasses  
                [~, ~, ~, ~, deltaMz0,...
                 deltaSz0] = tagi.hiddenStateBackwardPass(net, thetaClass{c},...
                 normStatClass{c}, stateClass{c}, deltaMzl, deltaSzl, [],...
                 maxIdx);
                [~, deltaMzlv] = tagi.detachMeanVar(deltaMz0, net.numCoord,...
                    net.numlv, net.batchSize, net.repBatchSize);
                [~, deltaSzlv] = tagi.detachMeanVar(deltaSz0, net.numCoord,...
                ...
                mlv(c, :) = mlv(c, :) + sum(deltaMzlv, 2)';
                Slv(c, :) = Slv(c, :) + sum(deltaSzlv, 2)';
            end
        end
        
        % Reinforcement learning 
        function [thetaP, normStatP] = DQN1net(netP, thetaP, thetaT, statesP,...
                normStatP, maxIdxP, state, action, nextState, reward, finalState)
            % Get updated indices for last layer
            udIdx = dp.selectIndices(action, netP.batchSize * netP.repBatchSize,...
                netP.rl.numActions, netP.dtype);    
            
            % Get next Q values
            statesT = tagi.initializeInputs(statesP, nextState, [], [], [],...
                [], [], [], [], [], netP.xsc);                        
            statesT = tagi.feedForwardPass(netP, thetaT, normStatP, statesT,...
                maxIdxP);               
            [~, ~, nextMq, nextSq] = tagi.extractStates(statesT);
            nextMq = reshape(nextMq{end}, [netP.ny, netP.batchSize * netP.repBatchSize]);
            nextSq = reshape(nextSq{end}, [netP.ny, netP.batchSize * netP.repBatchSize]);  
            [nextMq, nextSq] = rltagi.nextQvalues(nextMq, nextSq, netP.ny);
            ...
            % Get observation
            y  = netP.rl.gamma * nextMq + reward;
            Sy = (netP.rl.gamma^2) * nextSq;      
            
            % Update policy network
            [deltaM, deltaS, deltaMx, ...
             deltaSx] = tagi.hiddenStateBackwardPass(netP, thetaP, normStatP,...
             statesP, y, Sy, udIdx, maxIdxP);
            dthetaP = tagi.parameterBackwardPass(netP, thetaP, normStatP,...
                statesP, deltaM, deltaS, deltaMx, deltaSx);
            thetaP  = tagi.globalParameterUpdate(thetaP, dthetaP, netP.gpu);
        end 
        function [thetaP, normStatP] = DQN1netFullCovCUDA(netP, thetaP,...
                thetaT, statesP, normStatP, maxIdxP, ms, Ssd, Ssf, a, mns,...
                Snsd, Snsf, r, finalState)
            % Get updated indices for last layer
            udIdx = dp.selectIndices(a, netP.batchSize * netP.repBatchSize,...
                netP.rl.numActions, netP.dtype);    
            
            % Get next Q values
            statesT = tagi.initializeInputs(statesP, mns, Snsd, [], [], [],...
                [], [], [], [], netP.xsc);                        
            statesT = tagi.feedForwardPassFullCovCUDA(netP, thetaT,...
                normStatP, statesT, maxIdxP, Snsf);               
            [~, ~, nextMq, nextSq] = tagi.extractStates(statesT);
            nextMq = reshape(nextMq{end}, ...
                [netP.ny, netP.batchSize * netP.repBatchSize]);
            ...
            % Get current Q
            netP.trainMode = true;
            statesP = tagi.initializeInputs(statesP, ms, Ssd, [], [], [], ...
                [], [], [], [], netP.xsc);
            [statesP, normStatP, ...
                maxIdxP] = tagi.feedForwardPassFullCovCUDA(netP, thetaP, ...
                normStatP, statesP, maxIdxP, Ssf);
            
            % Get observation
            y  = netP.rl.gamma * nextMq + r;
            Sy = (netP.rl.gamma^2) * nextSq;      
            
            % Update policy network
            [deltaM, deltaS, deltaMx, ...
                deltaSx] = tagi.hiddenStateBackwardPass(netP, thetaP, ...
                normStatP, statesP, y, Sy, udIdx, maxIdxP);
            dthetaP = tagi.parameterBackwardPass(netP, thetaP, normStatP, ...
                statesP, deltaM, deltaS, deltaMx, deltaSx);
            thetaP  = tagi.globalParameterUpdate(thetaP, dthetaP, netP.gpu);
        end   
        function [thetaQ, thetaA, normStatQ] = DQNCARL(netQ, netA, state,...
                action, nextState, nextAction, reward, finalState)
            actFunIdx = 1;
            ...
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            %%% Step 1: Update value network (netQ)
            % Get next-Q values
            [nextMq, nextSq] = rltagi.nstepValue_V2(netQ, netA, nextState, nextAction);
            nextMq(finalState) = zeros(1, 1, 'like', nextMq);
            nextSq(finalState) = zeros(1, 1, 'like', nextSq); 
            
            % Get Q values 
            [malA]  = act.meanVar(action, action, zeros(size(action)),...
                actFunIdx, netQ.actBound(1), netQ.batchSize, netQ.repBatchSize, netQ.gpu) ;  
            ...
            % Get an observation
            my = netQ.rl.gamma * nextMq + reward;
            Sy = (netQ.rl.gamma^2) * (nextSq);  
            
            % Update netQ
            [deltaMQ, deltaSQ, deltaMxQ, ...
                deltaSxQ] = tagi.hiddenStateBackwardPass(netQ, thetaQ, ...
                normStatQ, statesQ, my, Sy, [], maxIdxQ);
            dthetaQ = tagi.parameterBackwardPassCUDA(netQ, thetaQ, ...
                normStatQ, statesQ, deltaMQ, deltaSQ, deltaMxQ, deltaSxQ); 
            thetaQ = tagi.globalParameterUpdate(thetaQ, dthetaQ, netQ.gpu);
            
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            %%% Step 2: Update policy network (netA)
            % Get Q values                    
            ...
            [statesQ, normStatQ, maxIdxQ,...
                mdaQ, SdaQ] = tagi.feedForwardPassCUDA(netQ, thetaQ, ...
                normStatQ, statesQ, maxIdxQ);
            ...
            [deltaMA, deltaSA, deltaMxA, ...
                deltaSxA] = tagi.hiddenStateBackwardPass(netA, thetaA, ...
            ...
            thetaA = tagi.globalParameterUpdate(thetaA, dthetaA, netA.gpu);
        end       
        function [dthetaQ, normStatQ, malQ] = tagiOptNstepQ(netQ, netA,...
            ...
            dthetaQ = tagi.parameterBackwardPass(netQ, thetaQ, normStatQ,...
        ...
        function [dthetaA, numOppSign] = tagiOptA(netQ, netA, state)
            ...
            [statesQ, normStatQ, maxIdxQ,...
                mdaQ, SdaQ] = tagi.feedForwardPass(netQ, thetaQ, normStatQ,...
                statesQ, maxIdxQ);
            [mdgQ, SdgQ, CdgzQ] = tagi.derivative(netQ, thetaQ, normStatQ,...
                statesQ, mdaQ, SdaQ, dlayerQ);
            ...
            [deltaMA, deltaSA, deltaMxA,...
                deltaSxA] = tagi.hiddenStateBackwardPass(netA, thetaA,...
                normStatA, statesA, malA, SalA, [], maxIdxA);
            dthetaA = tagi.parameterBackwardPass(netA, thetaA, normStatA,...
                statesA, deltaMA, deltaSA, deltaMxA, deltaSxA);
        end 
...
%                     % Error rate
        ...
        function [theta, normStat, zl, Szl, sv] = regressionFullCov(net,...
                theta, normStat, states, maxIdx, x, Sxf, y)
            % Initialization
            numObs = size(x, 1);
            numDataPerBatch = net.repBatchSize * net.batchSize;
            zl  = zeros(numObs, net.ny, net.dtype);
            ...
            for i = 1 : numDataPerBatch : numObs
                ...
                xloop    = reshape(x(idxBatch, :)', ...
                    [net.batchSize * net.nx, net.repBatchSize]);
                Sxfloop1 = reshape(Sxf(idxBatch, :)', ...
                    [net.batchSize * net.nx, net.repBatchSize]);
                Sxfloop  = Sxfloop1(:); % It must to modify with nx > 1
                states   = tagi.initializeInputs(states, xloop, Sxfloop1, [],...
                    [], [], [], [], [], [], net.xsc);
                % Training
                if net.trainMode                 
                    % Observation
                    ...
                    if ~net.cuda
                        [states, normStat, ...
                            maxIdx] = tagi.feedForwardPassFullCov(net,...
                            theta, normStat, states, maxIdx, Sxfloop);
                    else
                        [states, normStat, ...
                            maxIdx] = tagi.feedForwardPassFullCovCUDA(net,...
                            theta, normStat, states, maxIdx, Sxfloop);
