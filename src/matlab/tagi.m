 ...
 classdef tagi
    methods(Static) 
        ...
        function [states, normStat, maxIdx,...
                mda, Sda] = feedForwardPassFullCov(net, theta, normStat,...
                states, maxIdx, Sxf)
            % Initialization
            [mw, Sw, mb, Sb, mwx, Swx,...
                mbx, Sbx] = tagi.extractParameters(theta);
            ...
            for j = 2:numLayers
                ...
                elseif layer(j) == net.layerEncoder.ln...
                    ...
                    mhatD = tagi.distributeNormMeanVar(mra{j-1}, nodes(j-1),...
                        imgW(j-1), imgH(j-1), filter(j-1), B, rB, layer(j-1),...
                        layer(j), net.layerEncoder);
                    ShatD = tagi.distributeNormMeanVar(Sra{j-1}, nodes(j-1),...
                        imgW(j-1), imgH(j-1), filter(j-1), B, rB, layer(j-1),...
                        layer(j), net.layerEncoder);
                    if layer(j-1) == net.layerEncoder.fc
                ...
                elseif layer(j) == net.layerEncoder.conv 
                    if B == 1 && rB == 1
                        [mz{j}, Sz{j}] = tagi.convMeanVarB1(mw(idxw),...
                            Sw(idxw), mb(idxb), Sb(idxb), ma{j-1}, Sa{j-1},...
                            net.idxFmwa(smlIdx(j-1), :),...
                            kernelSize(j-1), filter(j-1), imgW(j), imgH(j),...
                            filter(j), net.padding(j-1), net.gpu);
                    else
                        [mz{j}, Sz{j}] = tagi.convMeanVar(mz{j}, Sz{j},...
                            mw(idxw), Sw(idxw), mb(idxb), Sb(idxb), ma{j-1},...
                            Sa{j-1}, net.idxFmwa(smlIdx(j-1), :),...
                ...
                % Full-connected
                elseif layer(j) == net.layerEncoder.fc
                    [mz{j}, Sz{j}, Szf{j}] = tagi.fcMeanCov(mw(idxw),...
                        Sw(idxw), mb(idxb), Sb(idxb), ma{j-1}, Sa{j-1},...
                        Saf{j-1}, nodes(j-1), nodes(j), B, net.gpu);                   
                end     
                ...
                if actFunIdx(j) ~= 0
                    [ma{j}, Sa{j}, J{j}] = act.meanVar(mz{j}, mz{j}, Sz{j},...
                        actFunIdx(j), actBound(j), B, rB, net.gpu);
                    [Saf{j}] = act.cov(J{j}, Szf{j}, nodes(j), B);
                else
                    ma{j}  = mz{j};
                    Sa{j}  = Sz{j};
                    J{j}   = ones(size(mz{j}), 'like', mz{j});  
...
%                 % Shortcut connection for residual networks 
        ...
        function [states, normStat, maxIdx,...
                mda, Sda] = feedForwardPassFullCovCUDA(net, theta, normStat,...
                states, maxIdx, Sxf)
            % Initialization
            [mw, Sw, mb, Sb, mwx, Swx,...
                mbx, Sbx] = tagi.extractParameters(theta);
            ...
            for j = 2:numLayers                  
                ...
                elseif layer(j) == net.layerEncoder.conv    
                    [mz{j}, Sz{j}] = convMeanVar4matlab(mw, Sw, mb, Sb,...
                        ma{j-1}, Sa{j-1}, net.idxFmwa{smlIdx(j-1), 2},...
                        numParamsPerlayer_2(1, j-1),...
                        numParamsPerlayer_2(2, j-1),...
                        paramUpdateIdx(2, j-1), imgW(j), imgH(j), filter(j),...
                        imgW(j-1), imgH(j-1), filter(j-1), kernelSize(j-1), B);
                ...
                % Full-connected
                elseif layer(j) == net.layerEncoder.fc
                    [mz{j}, Sz{j}, Szf{j}] = fcMeanCov4matlab_V2(mw, Sw, mb,...
                        Sb, ma{j-1}, Sa{j-1}, Saf{j-1},...
                        numParamsPerlayer_2(1, j-1),...
                        numParamsPerlayer_2(2, j-1), nodes(j), nodes(j-1), B);
                end     
                ...
                if net.xsc(j) ~= 0 && ...
                    ...
                    [mxs{j}, Sxs{j}] = convMeanVar4matlab(mwx, Swx, mbx,...
                        ...
                        numParamsPerlayer_2(4, idxXsc),...
                        paramUpdateIdx(4, idxXsc), imgW(j), imgH(j),...
                        filter(j), imgW(idxXsc), imgH(idxXsc), filter(idxXsc),...
                        1, B);
                    % Save convolutional hidden state before adding x
                    % shortcut
                    mdxs{j} = mz{j};
                ...
                % Activation
                [ma{j}, Sa{j}, J{j}, Saf{j}]=actMeanCov4matlab_V2(mz{j},...
                    Sz{j}, Szf{j}, net.lreluRate, actFunIdx(j), nodes(j), B);
...
%                              Szv, J{lHL+1}.*Sz{lHL+1}, y, net.gpu);
        ...
        % Visual field
        function [mz, Sz, prob] = vfFeedForwardPass(mz0, Sz0, mwg, Swg,...
                levelNodes, B)
            classObs = 1;
            classIdx = 1;
            numLevels = length(levelNodes);
            mz    = cell(numLevels, 1);
            Sz    = cell(numLevels, 1);
            prob  = cell(numLevels, 1);
            mz{1} = mz0;
            Sz{1} = Sz0;           
            for j = 2:numLevels               
                mzloop   = zeros(B, levelNodes(j), 'like', mz0);
                Szloop   = mzloop;
                probloop = mzloop;
                for i = 1 : levelNodes(j)
                    idxloop = i : i + 2 - 1;
                    probloop(i) = tagi.obs2class(mwg{j-1}(i), Swg{j-1}(i),...
                        classObs, classIdx);
                    [mzloop(:, i),...
                        Szloop(:, i)] = tagi.vfMeanVar(mz{j-1}(:, idxloop),...
                        Sz{j-1}(:, idxloop), probloop(i));
                end
                mz{j}   = mzloop;
                Sz{j}   = Szloop;
                prob{j} = probloop;
        ...
        function [deltaMwg, deltaSwg] = vfParameterBackwardPassMain(mz, mwg,...
                Swg, deltaM, deltaS, levelNodes)
            % Initialization
            numLevels = length(levelNodes);
            deltaMwg  = cell(numLevels, 1);
            deltaSwg  = cell(numLevels, 1);            
            for k = numLevels-1 : -1 : 1
                deltaMwgloop = zeros(nodes(k+1), 1, 'like', mwg{k});
                deltaSwgloop = deltaMwgloop ;
                
                % Update gate's parameters
                for i = 1 : levelNodes(k+1)
                    idxloop = i : i + 2 - 1;
                    [deltaMwgloop(i),...
                     deltaSwgloop(i)] = tagi.vfParameterBackwardPass(mz{k}(:, idxloop),...
                     mwg{k}(i), Swg{k}(i), deltaM{k+1}(i), deltaS{k+1}(i));
                end
                
                % Store results
                deltaMwg{k} = deltaMwgloop;
                deltaSwg{k} = deltaSwgloop;
            end
        end
        function [deltaMz0, deltaSz0,...
                deltaM, deltaS] = vfHiddenStateBackwardPassMain(Sz, prob,...
                deltaMzl, deltaSzl, levelNodes, B)
            % Initialization
            numLevels = length(levelNodes);
            deltaM    = cell(numLevels, 1);
            deltaS    = cell(numLevels, 1);
            deltaMz   = cell(numLevels, 1);
            deltaSz   = cell(numLevels, 1);
            deltaMz{end} = deltaMzl;
            deltaSz{end} = deltaSzl;
            
            for k = (numLevels-1) : -1 : 1
                % Compute inovation vector
                [deltaM{k+1}, deltaS{k+1}] = tagi.inovationVector(Sz{k+1},...
                    deltaMz{k+1}, deltaSz{k+1}, net.gpu);
                deltaMzloop = zeros(B, nodes(k), 'like', deltaMz);
                deltaSzloop = deltaMzloop;
                
                % Update hidden states
                for i = 1:nodes(k+1)
                    idxloop = i : i + 2 - 1;
                    [deltaMzloop(:, idxloop),...
                     deltaSzloop(:, idxloop)] = tagi.vfHiddenStateBackwardPass(Sz{k}(:, idxloop),...
                     prob(i), deltaM{k+1}(i), deltaS{k+1}(i));
                end
                
                % Store results
                deltaMz{k} = deltaMzloop;
                deltaSz{k} = deltaSzloop;
                
                deltaMz0 = deltaMz{1};
                deltaSz0 = deltaSz{1};
            end
        end
        function classProb = obs2class(mwg, Swg, classObs, classIdx)          
            alpha     = 3;
            samples   = mwg ./ sqrt((1 / alpha) ^ 2 + Swg);
            obsProb   = normcdf(samples, 0, 1);           
            classProb = prod(abs(obsProb(classIdx) - (classObs == -1)), 2);
        end 
        function [mznext, Sznext] = vfMeanVar(mz, Sz, prob)
            mznext = prob * mz(1, :) + (1 - prob) .* mz(2, :);
            Sznext = (prob .^2) * Sz(1, :) +  ((1 - prob) .^2) * Sz(2, :);
        end
        function [deltaMwg, deltaSwg] = vfParameterBackwardPass(mz, mwg,...
                Swg, deltaM, deltaS)
            Jwg      = normpdf(mwg);
            Cwgy     = Jwg .* Swg .* (mz(:, 1) - mz(:, 2));
            deltaMwg = sum(Cwgy .* deltaM, 1);
            deltaSwg = sum(Cwgy .* deltaS .* Cwgy, 1);
        end
        function [deltaMz, deltaSz] = vfHiddenStateBackwardPass(Sz, prZ1,...
                deltaM, deltaS)
            Cz1y     = Sz(:, 1) .* prZ1;
            Cz2y     = Sz(:, 2) .* prZ1 + Sz(:, 2);
            deltaMz1 = Cz1y .* deltaM;
            deltaSz1 = Cz1y .* deltaS .* Cz1y;
            deltaMz2 = Cz2y .* deltaM;
            deltaSz2 = Cz2y .* deltaS .* Cz2y;
            deltaMz  = [deltaMz1, deltaMz2];
            deltaSz  = [deltaSz1, deltaSz2];
        end
        function [mwg, Swg] = globalGateParameterUpdate(mwg, Swg, deltaMwg,...
                deltaSwg)
            numLevels = size(mwg, 1);
            for j = 1:numLevels
                mwg{j} = mwg{j} + deltaMwg{j};
                Swg{j} = Swg{j} + deltaSwg{j};
            end
        end        
        function [mwg, Swg] = initializeGateParameter(levelNodes, dtype)
            numLevels = length(levelNodes);
            mwg       = cell(numLevels, 1);
            Swg       = cell(numLevels, 1);
            for j = 2:numLevels
                mwg{j-1} = zeros(levelNodes(j), 1, dtype);
                Swg{j-1} = 0.5 * ones(levelNodes(j), 1, dtype);
            end
        end
